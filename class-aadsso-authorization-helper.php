<?php
/**
 * AADSSO Authorization Helper: class-aadsso-authorization-helper.php
 *
 ...
 */
class AADSSO_Authorization_Helper {
	/**
	 * Allowable algorithms from Azure Active Directory.  Only RS256 is allowed and expected from AAD.
	 *
	 * @var array The array of allowed algorithms from Azure Active Directory.
	 */
	 ...
	 * @param \AADSSO_Settings $settings The settings to use.
	 * @param string           $antiforgery_id The antiforgery_id nonce.
	 *
	 * @throws DomainException When $discover->keys is null (problem with JWT response).
	 *
	 * @return mixed The decoded authorization result.
	 */
	public static function validate_id_token( $id_token, $settings, $antiforgery_id ) {
		...
		// TODO: cache the keys found in jwks.
		...
		if ( $jwt->nonce !== $antiforgery_id ) {
			// translators: This message is shown to the user when an error matching their antiforgery_id occurs.
			throw new DomainException( sprintf( __( 'Nonce mismatch. Expecting %s', 'aad-sso-wordpress' ), $antiforgery_id ) );
		}
