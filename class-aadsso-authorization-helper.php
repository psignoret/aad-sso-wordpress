<?php
/**
 * File class-authorization-helper.php
 *
...
class AADSSO_Authorization_Helper {
	...
	public static function get_and_process_access_token( $authentication_request_body, $settings ) {
		...
		/*
			Decode the JSON response from the STS. If all went well, this will contain the
			access token and the id_token (a JWT token telling us about the current user).
		*/
	 ...
	 *
	 * @param string           $id_token The id_token to validate.
	 * @param \AADSSO_Settings $settings The settings to use.
	 * @param string           $antiforgery_id The value to use as the nonce.
	 *
	 * @throws DomainException If the id_token is invalid.
	 *
	 * @return mixed The decoded authorization result.
	 */
	public static function validate_id_token( $id_token, $settings, $antiforgery_id ) {
		...
		// Use wp transient cache to get the keys, otherwise store them in the db for an hour.
		// Allow this to be reset by passing aadsso_reset_jwks in the query string.
		$jwks = get_transient( 'aadsso_jwks' );
		if ( false === $jwks || array_key_exists( 'aadsso_reset_jwks', $_GET ) ) {
			$jwks = wp_remote_retrieve_body( wp_remote_get( $settings->jwks_uri ) );
			set_transient( 'aadsso_jwks', $jwks, 60 * 60 );
		}
