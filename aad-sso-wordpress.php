 ...
 *
 * Configures, hooks and loads the Azure Active Directory Single Sign-On plugin.
 */
class AADSSO {
	 ...
	 * AADSSO Instance
	 *
	 * @var \AADSSO singleton instance of the AADSSO, ensuring one instantiation
	 */
	static public $instance = false;

	/**
	 * AADSSO Settings Instance
	 *
	 * @var \AADSSO_Settings singleton instance of the current settings class
	 */
	private $settings = null;

	/**
	 * Constructs and initalizes the AADSSO plugin: adds hooks and filters and configures based on $settings
	 *
	 * @param \AADSSO_Settings $settings A properly-instantiated AADSSO_Settings instance.
	 */
	public function __construct( $settings ) {
		...
		*/
		...
		// Adds a link to the Settings page in the list of plugins.
		add_filter(
	 ...
	 *
	 * @param \AADSSO_Settings $settings  Configuration object, must extend AADSSO_Settings.
	...
	public function save_redirect_and_maybe_bypass_login() {
		...
		if ( $this->wants_to_login() ) {
			...
			// Saves the redirect_to query param ( if present ) to session.
	 ...
	 *
	 * @param string           $redirect_to URL to restore the redirection to.
	 * @param string           $requested_redirect_to Unused, but provided to comply with hook `login_redirect`.
	 * @param WP_User|WP_Error $user The logged-in user, if any.
	 *
	 * @return string
	 */
	public function redirect_after_login( $redirect_to, $requested_redirect_to, $user ) {
		if (
			is_a( $user, 'WP_User' )
			&& isset( $_SESSION['aadsso_redirect_to'] )
		) {
	...
	private function wants_to_login() {
		...
		// Covers default WordPress behavior.
		$action = isset( $_REQUEST['action'] )
			? $_REQUEST['action']
			: 'login';

		// And now the exceptions.
		$action = isset( $_GET['loggedout'] )
			? 'loggedout'
			: $action;
		...
		if ( isset( $_GET['code'] ) ) {
			...
			if ( $state_is_missing || $state_doesnt_match ) {
				return new WP_Error(
					...
					// translators: This message appears when there is an error validating a security token.
			...
			$token = AADSSO_Authorization_Helper::get_access_token( $_GET['code'], $this->settings );
			...
			return new WP_Error(
				$_GET['error'],
				sprintf(
					// translators: Message is displayed when Azure Active Directory refuses the request from WordPress.
	...
	/**
	 * Attempts to lookup a WordPress user from an AAD response.
	 *
	 * @param object $jwt The JSON Web Token respons from AAD.
	 * @return WP_User|WP_Error Returns a WP_User, if found, otherwise WP_Error
	 */
	public function get_wp_user_from_aad_user( $jwt ) {
		...
		$unique_name = isset( $jwt->upn )
			? $jwt->upn
			: isset( $jwt->unique_name )
				? $jwt->unique_name
				: null;
		...
		if ( is_a( $user, 'WP_User' ) ) {
			AADSSO::debug_log(
				sprintf(
					'Matched Azure AD user [%s] to existing WordPress user [%s].', $unique_name, $user->ID
				), 10
			);
		} else {
			...
			if ( true === $this->settings->enable_auto_provisioning ) {
				...
				if ( is_wp_error( $new_user_id ) ) {
					...
					return new WP_Error(
						'user_not_registered',
						sprintf(
							// translators: Message is displayed when user, %1$s, cannot be created.
							__( 'ERROR: Error creating user \'%1$s\'.', 'aad-sso-wordpress' ),
							$unique_name
						)
					);
				} else {
				...
				return new WP_Error(
					'user_not_registered',
					sprintf(
						// translators: Message displayed when user, %1$s is not registered on this site.
						__(
							'ERROR: The authenticated user \'%1$s\' is not a registered user in this site.',
							'aad-sso-wordpress'
						),
						$unique_name
					)
	 ...
	 */
	private function update_wp_user_roles( $user, $aad_user_id, $aad_tenant_id ) {

		// TODO: Cleaner (but still lazy) initialization of GraphHelper.
		AADSSO_Graph_Helper::$settings  = $this->settings;
		AADSSO_Graph_Helper::$tenant_id = $aad_tenant_id;
		...
		$group_memberships = AADSSO_Graph_Helper::user_check_member_groups( $aad_user_id, $group_ids );
		...
		if ( isset( $group_memberships->value ) ) {
			AADSSO::debug_log(
				sprintf(
					...
					implode( ',', $group_ids ), $aad_user_id, implode( ',', $group_memberships->value )
				), 20
			);
			...
			return new WP_Error(
				...
				sprintf(
					// translators: Message shown when an error checking group memberships is encountered.
					__(
						'ERROR: Unable to check group membership in Azure AD: <strong>%1$s</strong>.',
						'aad-sso-wordpress'
					), $group_memberships->{'odata.error'}->code
				)
			);
		} else {
			...
			return new WP_Error(
				...
				__(
					'ERROR: Unexpected response when checking group membership in Azure AD.',
					'aad-sso-wordpress'
				)
			);
		}
		...
		if ( ! empty( $roles_to_set ) ) {
			...
			AADSSO::debug_log(
				sprintf(
					'Set roles [%s] for user [%s].', implode( ', ', $roles_to_set ), $user->ID
				), 10
			);
			...
			AADSSO::debug_log(
				sprintf(
					'Set default role [%s] for user [%s].', $this->settings->default_wp_role, $user->ID
				), 10
			);
		} else {
			$error_message = sprintf(
				// translators: Message is shown when user identified by %1$s is not a member of a group that grants them a role on this WordPress site.
				__( 'ERROR: Azure AD user %1$s is not a member of any group granting a role.', 'aad-sso-wordpress' ),
				$aad_user_id
			);
	...
	public function add_settings_link( $links ) {
		$link_to_settings = sprintf(
			'<a href="%1$s">%2$s</a>',
			esc_attr( admin_url( 'options-general.php?page=aadsso_settings' ) ),
			esc_html__( 'Settings', 'aad-sso-wordpress' )
		);
	...
	public function print_plugin_not_configured() {
		...
		. esc_html__(
			'Single Sign-on with Azure Active Directory required settings are not defined. '
	...
	public function print_login_link() {
		...
		$html .= '<a href="%s">';
		$html .= sprintf(
			// translators: Message is shown below the WordPress login form, specifying that the user can login to an account managed by the organization, %1$s.
			esc_html__( 'Sign in with your %1$s account', 'aad-sso-wordpress' ),
...
if ( ! function_exists( 'com_create_guid' ) ) {
	...
	function com_create_guid() {
		...
		$uuid   = chr( 123 ) // "{"
			...
			// TODO: ensure UUIDv4 Compliance? @psignoret?
